...
The BugTraceAI-CLI exposes a FastAPI REST and WebSocket control surface for scan management, findings, reports, configuration, and metrics. The API follows the OpenAPI 3.1 specification.

This page documents **BugTraceAI-CLI**. [BugTraceAI-API](/bugtraceai-api) is a separate REST/MCP API-security service with its own scan model, evidence store, and OpenAPI schema.

---

## Base URL

`CLI_BASE_URL` is the CLI address printed by the Launcher or selected for a direct deployment. Do not hard-code a port in client code.
...
```bash
curl -o report.zip "$CLI_BASE_URL/api/scans/{id}/report-zip"
```
...
- **Swagger UI**: `${CLI_BASE_URL}/docs`
- **ReDoc**: `${CLI_BASE_URL}/redoc`
- **JSON Spec**: `${CLI_BASE_URL}/openapi.json`

---

**Parent**: [Architecture](/architecture)

**See also**: [WebSocket Events](/websocket-events) | [Configuration](/configuration) | [BugTraceAI-CLI](/bugtraceai-cli) | [BugTraceAI-API](/bugtraceai-api)
